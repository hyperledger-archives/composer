...
class JSONGenerator {
    ...
    convertToJSON(field, obj) {
        if(field instanceof ClassDeclaration) {
            throw new Error( 'Cannot convert ' + field.getFullyQualifiedTypeName() + ' directly to JSON for object ' + obj );
        }
    ...
    */
    getRelationshipText(relationshipDeclaration, relationship) {
        ...
        let relationshipText = relationship.getIdentifier();
        if(relationshipDeclaration.getNamespace() !==  relationship.getNamespace() ) {
            relationshipText = relationship.getFullyQualifiedIdentifier();
        }
