...
function getVisibility(comment) {
    ...
    let parsedComment = doctrine.parse(comment, {unwrap: true, tags: [PRIVATE]});
...
function getReturnType(comment) {
    ...
    let result = 'void';
    let parsedComment = doctrine.parse(comment, {unwrap: true, tags: [RETURN, RETURNS]});
    ...
    tags.forEach((tag) => {
        ...
        result = tag.type.name ? tag.type.name : tag.type.applications ? tag.type.applications[0].name + '[]' : '';
    });
    return result;
}
...
function getThrows(comment) {
    ...
    let result = '';
    let parsedComment = doctrine.parse(comment, {unwrap: true, tags: [THROWS, EXCEPTION]});
...
function getMethodArguments(comment) {
    const TAG = 'param';
    let paramTypes = [];
    let parsedComment = doctrine.parse(comment, {unwrap: true, tags: [TAG]});
