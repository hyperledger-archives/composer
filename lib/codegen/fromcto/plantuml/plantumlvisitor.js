...
/**
 * Convert the contents of a ModelManager to PlantUML format files.
...
class PlantUMLVisitor {
    ...
    visitModelFile(modelFile, parameters) {
        ...
        const txProcessor = modelFile.getNamespace() + '.TransactionProcessor';
        parameters.fileWriter.writeLine(0, 'class ' + txProcessor + ' << (X,brown) >> {' );
        visitClass = function(functionDeclaration) {
            functionDeclaration.accept(this,parameters);
        };
        _.each(modelFile.getFunctions(), visitClass.bind(this));
        parameters.fileWriter.writeLine(0, '}' );

        const functions = modelFile.getFunctions();
        for(let n=0; n < functions.length; n++ ) {
            const functionDeclaration = functions[n];
            const txName = modelFile.getNamespace() + '.' + functionDeclaration.getTransactionDeclarationName();
            if(txName) {
