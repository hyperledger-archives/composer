...
class HFCUtil {
    ...
    static deployChainCode(securityContext, chaincodePath, functionName, args, force) {
        ...
        .then((tempDirectoryPath) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                fs.copy(sourceDirectoryPath, targetDirectoryPath, { filter: (path) => { return !/ibm-concerto-runtime-hlf.*node_modules/.test(path); }}, (err) => {
                    if (err) {
    ...
    /**
     * Submit an undeploy request to the chain-code
     * @param {HFCSecurityContext} securityContext - The user's security context
     * @param {string[]} args - The arguments to pass to the function being called.
     * @return {Promise} - A promise that will be resolved with the value returned
     * by the chain-code function.
     */
    static undeployBusinessNetworkDefinition(securityContext, args) {
        HFCUtil.securityCheck(securityContext);
        return HFCUtil.invokeChainCode('undeployBusinessNetworkDefinition', args);
    }
     ...
     * by the chain-code function.
     */
    static updateBusinessNetworkDefinition(securityContext, args) {
        HFCUtil.securityCheck(securityContext);
        return HFCUtil.invokeChainCode('updateBusinessNetworkDefinition', args);
    }
