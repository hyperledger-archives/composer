...
class HFCConnection extends Connection {
    ...
    login(enrollmentID, enrollmentSecret) {
        ...
        .then((securityContext) => {
            return self.getConnectionManager().getConnectionProfileManager().getConnectionProfileStore().load(self.connectionProfile)
                .then((profile) => {
                    if (this.businessNetworkIdentifier && profile.networks && profile.networks[this.businessNetworkIdentifier]) {
                        securityContext.setChaincodeID(profile.networks[this.businessNetworkIdentifier]);
                    }
                })
                .then(() => {
    ...
    deploy(securityContext, force, businessNetwork) {
        ...
        return businessNetwork.toArchive()
            ...
            .then((profile) => {
                ...
                profile.networks[businessNetwork.getIdentifier()] = chaincodeId;
            ...
            .then(() => {
                ...
                self.businessNetworkIdentifier = businessNetwork.getIdentifier();
                LOG.info('deploy', 'BusinessNetwork identifier modified', self.getIdentifier());
    ...
    undeploy(securityContext, businessNetworkIdentifier) {
        ...
        if (!self.connectionProfile || !businessNetworkIdentifier) {
            throw new Error('Connection profile and business network must be specified.');
        }

        return self.getConnectionManager().getConnectionProfileManager().getConnectionProfileStore().load(self.connectionProfile)
        .then((prof) => {
            profile = prof;
            if (!profile || !profile.networks || !profile.networks[businessNetworkIdentifier]) {
                throw new Error('BusinessNetwork ' + businessNetworkIdentifier + ' not found in connection profile. Has it been deployed?');
            }
        })
        .then(() => {
            return HFCUtil
              .undeployChainCode(securityContext, [businessNetworkIdentifier]);
        })
        .then(() => {
    ...
    update(securityContext, businessNetworkDefinition) {
        ...
        return businessNetworkDefinition.toArchive()
            .then((buffer) => {
                return HFCUtil
                    .updateBusinessNetworkDefininition(securityContext, [buffer.toString('base64')]);
            });
    }
