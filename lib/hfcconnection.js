...
class HFCConnection extends Connection {
    ...
    constructor(connectionManager, connectionProfile, businessNetworkIdentifier, chain) {
        ...
        if (businessNetworkIdentifier && businessNetworkIdentifier.indexOf('-') !== -1) {
            throw new Error('Business network identifier cannot (yet) contain a version specified. Please remove the version number.');
        }
                ...
                return self.getConnectionManager().getConnectionProfileManager().getConnectionProfileStore().load(self.connectionProfile)
                    .then((profile) => {
                        ...
                        if (self.businessNetworkIdentifier) {
                            let chaincodeID = profile.networks[self.businessNetworkIdentifier];
                            if (chaincodeID) {
                                securityContext.setChaincodeID(profile.networks[self.businessNetworkIdentifier]);
                            } else {
    ...
    undeploy(securityContext, businessNetworkIdentifier) {
        ...
        if (businessNetworkIdentifier.indexOf('-') !== -1) {
            throw new Error('Business network identifier cannot (yet) contain a version specified. Please remove the version number.');
        }
    ...
    /**
     * Register a new user with the specified enrollment ID and attributes.
     * @param {SecurityContext} securityContext The participant's security context.
     * @param {string} enrollmentID The new users enrollment ID.
     * @return {Promise} A promise that is resolved with a generated enrollment
     * secret once the new user has been registered, or rejected with an error.
     */
    registerUser(securityContext, enrollmentID) {
        const method = 'registerUser';
        LOG.entry(method, securityContext, enrollmentID);
        HFCUtil.securityCheck(securityContext);
        return HFCUtil.registerUser(securityContext, enrollmentID)
