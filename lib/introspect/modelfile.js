...
class ModelFile {
    ...
    /**
     * Convert the specified JSON into an instance of a model file.
     * @param {ModelManager} modelManager - the ModelManager that manages this
     * ModelFile
     * @param {Object} modelFile - A serialized instance of a ModelFile.
     * @param {string} modelFile.definitions - The definitions for the ModelFile.
     * @return {ModelFile} An instance of a model file.
     */
    static fromJSON(modelManager, modelFile) {
        return new ModelFile(modelManager, modelFile.definitions);
    }
    ...
    constructor(modelManager, definitions) {
        ...
        for(let n=0; n < this.ast.body.length; n++ ) {
            ...
            else if(thing.type === 'FunctionDeclaration') {
                this.functions.push( new FunctionDeclaration(this,thing));
            }
            else {
    ...
    validate() {
        ...
        for(let n=0; n < this.functions.length; n++) {
            const functionDeclaration = this.functions[n];
    ...
    /**
     * Convert this model file into an object that is suitable for converting
    ...
    toJSON() {
        return {
            ...
            assets: this.getAssetDeclarations(),
            participants: this.getParticipantDeclarations(),
            transactions: this.getTransactionDeclarations(),
            enums: this.getEnumDeclarations()
        };
    }
