...
class Registry {
    ...
    add(resource) {
        ...
        let data = this.serializer.toJSON(resource);
        return Util.invokeChainCode(this.securityContext, 'addResourceToRegistry', [this.registryType, this.id, resource.getIdentifier(), JSON.stringify(data)]);
    }
    ...
    addAll(resources) {
        ...
        let data = resources.map((resource) => {
            return {
                ...
                data: JSON.stringify(this.serializer.toJSON(resource))
            };
        });
        return Util.invokeChainCode(this.securityContext, 'addAllResourcesToRegistry', [this.registryType, this.id, JSON.stringify(data)]);
    }
    ...
    update(resource) {
        ...
        let data = this.serializer.toJSON(resource);
        return Util.invokeChainCode(this.securityContext, 'updateResourceInRegistry', [this.registryType, this.id, resource.getIdentifier(), JSON.stringify(data)]);
    }
    ...
    updateAll(resources) {
        ...
        return Util.invokeChainCode(this.securityContext, 'updateAllResourcesInRegistry', [this.registryType, this.id, JSON.stringify(data)]);
    }
    ...
    find(expression) {
        ...
        return Util.queryChainCode(this.securityContext, 'findResourcesInRegistry', [this.registryType, this.id, expression])
            ...
            .then((resources) => {
                return resources.map((resource) => {
                    return this.serializer.fromJSON(JSON.parse(resource.data));
                });
            });
    }
