 ...
 */
  ...
  /**
   * @description Log a message at the _silly_level
   *
   ...
   * @private
   */
    silly(method, msg, data) {
        this.intlog('silly',method,msg,data);
    }
   ...
   */
    static getLog(classname) {
        ...
        if(_logger === null) {
            ...
            if (config.has('ConcertoConfig.debug.logger')){
                let loggerToUse = config.get('ConcertoConfig.debug.logger');
                let myLogger = require((loggerToUse==='default')? './winstonInjector.js' : loggerToUse);
                _logger = myLogger.getLogger(config.get('ConcertoConfig.debug.config'));
            } else {
                // very basic fallback function to log to the console
                _logger =  {
                    log: function(){
        ...
        //
            ...
            // determine if the element is for concerto or not
                ...
                // got a result that we need to trace therefore setup the child node correctly
                    let newNode = new Node((machResult[1]===''),machResult[2]  );
