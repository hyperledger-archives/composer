    ...
    static fromArchive(Buffer) {
        return JSZip.loadAsync(Buffer).then(function(zip){
            ...
            if(packageJson.length !== 1){
                throw Error('There should only be one package.json');
            }

            packageJson.forEach(function(file){
                const packagePromise = file.async('string');
                ...
                packagePromise.then(contents => {
                    ...
                    businessNetwork = new BusinessNetwork(packageName+'-'+packageVersion, packageDescription);
                });
            });
            ...
            jsFiles.forEach(function(file){
                ...
                jsPromise.then(contents => {
                    let tempObj = {'name':file.name,'contents':contents};
            ...
            .then(() => {
                businessNetwork.modelManager.addModelFiles(ctoModelFiles); // Adds all cto files to model manager
                // console.log('What are the jsObjectsArray?',jsObjectArray);
                jsScriptFiles.forEach(function(obj){
                    let jsObject = businessNetwork.scriptManager.createScript(obj.name,'js', obj.contents);
                    businessNetwork.scriptManager.addScript(jsObject); // Adds all js files to script manager
                });

                return businessNetwork; // Returns business network (with model manager and script manager)
            });
        });
    }
