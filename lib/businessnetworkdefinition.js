...
class BusinessNetworkDefinition {
    ...
    static fromDirectory(path, options) {
        ...
        let packageJsonContents = fs.readFileSync( path + '/package.json', ENCODING);
        ...
        if(jsonObject.dependencies) {
            ...
            for( let dep of dependencies) {
                ...
                const dependencyPath = path + '/node_modules/' + dep;
        ...
        const isFileInNodeModuleDir = function(file) {
            ...
            let result = false;
            if( filePath.dir.substring(path.length).indexOf('node_modules/') >= 0) {
                result = true;
            }
    ...
    static processDirectory(path, fileProcessor) {
        const items = BusinessNetworkDefinition.walkSync(path);
        items.sort();
    ...
    static walkSync(dir, filelist) {
        ...
        files.forEach(function (file) {
            ...
            if (fs.lstatSync(nestedPath).isDirectory()) {
                filelist = BusinessNetworkDefinition.walkSync(nestedPath, filelist);
            } else {
