...
class HLFConnection extends Connection {
    ...
    async _checkEventHubStrategy() {
        ...
        // internal method to check for at least one connected event hub
        const checkForOne = () => {
        ...
        if (!checkForOne()) {
            ...
            // run a background connection on any disconnected hubs
            // to try to recover any that have disconnected or got into a state.
            for (const eventHub of this.eventHubs) {
                if (!HLFUtil.eventHubConnected(eventHub)) {
                    // run a connection in the background
                    eventHub.disconnect();
