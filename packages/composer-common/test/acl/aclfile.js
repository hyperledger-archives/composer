...
describe('AclFile', () => {
    ...
    describe('#validate', () => {
        ...
        it('should fail to validate a rule when resource is not an asset', () => {
            const aclContents = `rule C {
                description: "org.acme.Regulator is a participant, not an asset"
                participant: "ANY"
                operation: READ
                resource: "org.acme.Regulator"
                action: ALLOW
            }`;
            const aclFile = new AclFile('test.acl', modelManager, aclContents);
            (() => {
                aclFile.validate();
            }).should.throw(IllegalModelException, /The resource.*must be an asset/);
        });
