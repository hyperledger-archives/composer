...
class HLFConnectionManager extends ConnectionManager {
     ...
     * @return {Orderer} A new orderer.
     */
    static parseOrderer(orderer, timeout) {
        if (typeof orderer === 'object') {
            const opts = HLFConnectionManager._createOpts(orderer.cert, orderer.hostnameOverride, timeout);
     ...
     */
    static _createOpts(cert, override, timeout) {
        if (!timeout) {
     ...
     */
    static parsePeer(peer, timeout, eventHubDefs) {
        const method = 'parsePeer';

        const tlsOpts = HLFConnectionManager._createOpts(peer.cert, peer.hostnameOverride, timeout);
        ...
        const eventHub = HLFConnectionManager.createEventHubDefinition(peer.eventURL, tlsOpts);
    ...
    static createEventHubDefinition(eventURL, tlsOpts) {
        return {
            'eventURL': eventURL,
            'tlsOpts': tlsOpts
        };
    ...
    connect(connectionProfile, businessNetworkIdentifier, connectOptions) {
        ...
        if (!connectOptions.deployWaitTime) {
            connectOptions.deployWaitTime = 180;
        }
        if (!connectOptions.invokeWaitTime) {
            connectOptions.invokeWaitTime = 60;
        }

        const timeout = Math.max(connectOptions.deployWaitTime, connectOptions.invokeWaitTime);
        ...
        connectOptions.orderers.forEach((orderer) => {
            ...
            chain.addOrderer(HLFConnectionManager.parseOrderer(orderer, timeout));
        });
        ...
        connectOptions.peers.forEach((peer) => {
            ...
            chain.addPeer(HLFConnectionManager.parsePeer(peer, timeout, eventHubDefs));
        });
        ...
        return result.then((store) => {
            ...
            let connection = new HLFConnection(this, connectionProfile, businessNetworkIdentifier, connectOptions, client, chain, eventHubDefs, caClient);
