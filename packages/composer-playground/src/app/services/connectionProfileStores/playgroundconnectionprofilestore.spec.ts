...
describe('PlaygroundConnectionProfileStore', () => {
    ...
    describe('#load', () => {
        it('should load from the browser connection profile store', () => {
            mockBrowserConnectionProfileStore.load.withArgs('web-default').resolves({'x-type': 'web'});
            mockProxyConnectionProfileStore.load.rejects(new Error('such error'));
            return connectionProfileStore.load('web-default')
                .then((result) => {
                    result.should.deep.equal({'x-type': 'web'});
                });
        });
        ...
        it('should load from the proxy connection profile store if not in the browser connection profile store', () => {
            ...
            mockProxyConnectionProfileStore.load.resolves({'x-type': 'hlfv1'});
            return connectionProfileStore.load('hlfv1-default')
                .then((result) => {
                    result.should.deep.equal({'x-type': 'hlfv1'});
                });
        });
    });
    ...
    describe('#save', () => {
        it('should save web profiles to the browser connection profile store', () => {
            mockBrowserConnectionProfileStore.save.withArgs('web-default', {'x-type': 'web'}).resolves();
            mockProxyConnectionProfileStore.save.rejects(new Error('such error'));
            return connectionProfileStore.save('web-default', {'x-type': 'web'})
                .then((result) => {
                    sinon.assert.calledOnce(mockBrowserConnectionProfileStore.save);
                    sinon.assert.calledWith(mockBrowserConnectionProfileStore.save, 'web-default', {'x-type': 'web'});
        ...
        it('should save other profiles to the proxy connection profile store', () => {
            ...
            mockProxyConnectionProfileStore.save.withArgs('hlfv1-default', {'x-type': 'hlfv1'}).resolves();
            return connectionProfileStore.save('hlfv1-default', {'x-type': 'hlfv1'})
                .then((result) => {
                    ...
                    sinon.assert.calledWith(mockProxyConnectionProfileStore.save, 'hlfv1-default', {'x-type': 'hlfv1'});
                });
        });
    });
    ...
    describe('#delete', () => {
        ...
        it('should delete web profiles from the browser connection profile store', () => {
            sinon.stub(connectionProfileStore, 'load').withArgs('web-default').resolves({'x-type': 'web'});
        ...
        it('should delete other profiles from the proxy connection profile store', () => {
            sinon.stub(connectionProfileStore, 'load').withArgs('hlfv1-default').resolves({'x-type': 'hlfv1'});
            mockBrowserConnectionProfileStore.delete.rejects(new Error('such error'));
