...
let orangeUtils = require("./helpFull");
...
let businessNetworkDefinition, businessNetworkIdentifier, modelManager;

let participantList = [], assetList = [], assetServiceNames = [], assetComponentNames = [];
let transactionList = [], conceptList = [], transactionComponentNames = [], transactionServiceNames = [];

let enumerations, introspector, namespaceList;
let assetProperties, destinationPath, liveNetwork;

let skipInstall = false;

let appName, appDescription;
let authorName, authorEmail;
let license, networkIdentifier, connectionProfileName;
let enrollmentId, enrollmentSecret, apiServer;
let apiIP, apiPort, apiNamespace;
...
module.exports = yeoman.Base.extend({
    constructor: function () {
        ...
        if (arguments[1].skipInstall !== undefined) { skipInstall = arguments[1].skipInstall; }
        if (arguments[1].embeddedRuntime !== undefined) {
            businessNetworkConnection = new BusinessNetworkConnection({ fs: bfs_fs });
        } else { businessNetworkConnection = new BusinessNetworkConnection(); }
    },
            ...
            store: true
                ...
                if (liveNetwork) {
                    questions = [{
                        ...
                        type: 'input', name: 'appName', message: 'Project name:', default: 'angular-app',
                        store: true,
                        validate: Util.validateAppName
                    },{
                        type: 'input',
                        name: 'appDescription', message: 'Description:', default: 'Hyperledger Composer Angular project',
                        store: true,
                        ...
                        type: 'input', name: 'authorEmail', message: 'Author email:',
                        store: true,
                        validate: Util.validateAuthorEmail
                    },{
                        type: 'input', name: 'license', message: 'License:', default: 'Apache-2.0',
                        store: true,
                        validate: Util.validateLicense
                    },{
                        type: 'input', name: 'cardName', message: 'Name of the Business Network card:',
                        store: true,
                        validate: Util.cardName
                    },{
                        type: 'list', name: 'apiServer',
                        ...
                        validate: function (input) {
                            ...
                            else { return 'Connection Profile cannot be null or empty.';}
                        }
                    }];
                } else {
                    questions = [{
                        ...
                        type: 'input', name: 'appDescription', message: 'Description:',
                ...
                return this.prompt(questions).then(function (answers) {
                    ...
                    if (liveNetwork) {
                        ...
                        if (apiServer === 'generate') {
                            ...
                            nextQuestions = [{
                                type: 'input', name: 'apiPort', store: true,
                                message: 'REST server port:', default: '3000'
                            },{
                                type: 'list', name: 'apiNamespace',
                                message: 'Should namespaces be used in the generated REST API?',
                                default: 'never', store: true,
                                choices: [
                            ...
                            nextQuestions = [
                                { type: 'input', name: 'apiIP', store: true, message: 'REST server address:', default: 'http://localhost' },
                                { type: 'input', name: 'apiPort', store: true, message: 'REST server port:', default: '3000' },
                                { type: 'list', name: 'apiNamespace', message: 'Should namespaces be used in the generated REST API?',
                                    default: 'never', store: true,
                                    choices: [
                        ...
                        return self.prompt(nextQuestions).then(function (answers) {
                            if (apiIP === undefined) { apiIP = answers.apiIP; }
                        ...
                        nextQuestions = [
                            { type: 'input', name: 'apiIP', store: true, message: 'REST server address:', default: 'http://localhost' },
                            { type: 'input', name: 'apiPort', store: true, message: 'REST server port:', default: '3000' },
                            { type: 'list', name: 'apiNamespace', message: 'Are namespaces used in the generated REST API: ',
                                default: 'never', store: true,
                                choices: [
                                    { name: 'Namespaces are used', value: 'always' },
                                    { name: 'Namespaces are not used', value: 'never' }
    ...
    writing: function () {
        ...
        return completedApp.then(() => { console.log('Completed generation process'); });
    },
    ...
    _createApp: function () {
        ...
        let createdApp = new Promise((resolve, reject) => {
            ...
            namespaceList.forEach((namespace) => {
                ...
                assetDeclarations .filter((assetDeclaration) =>{
                    ...
                    assetProperties.forEach((property) => {
                        if (property.constructor.name === 'Field') {
                            if (property.isTypeEnum()) {
                                ...
                                tempList.push({ 'name': property.getName(), 'optional':property.optional ,'type': property.getType(), 'enum': true, 'isArray': property.array ,  'array': property.array === true, enumValues });
                            } else if (property.isPrimitive() || !property.isPrimitive()) {
                                tempList.push({ 'name': property.getName(), 'type': property.getType(), 'optional':property.optional, 'isArray': property.array});
                            } else { console.log('Unknown property type: ' + property); }
                        } else if (property.constructor.name === 'RelationshipDeclaration') {
                            tempList.push({ 'name': property.getName(), 'type': property.getType(), 'optional':property.optional, 'isArray': property.array });
                        } else { console.log('Unknown property constructor name: ' + property ); }
                    });

                    assetList.push({ 'name': asset.name, 'namespace': asset.getNamespace(), 'properties': tempList, 'identifier': asset.getIdentifierFieldName() });
                    shell.mkdir('-p', destinationPath + '/src/app/' + asset.name);

                });
            });

            modelManager.getParticipantDeclarations().forEach((data) => {
                if (data.fqn.indexOf("org.hyperledger.composer") == -1){ participantList.push(data) }
            })

            modelManager.getTransactionDeclarations().forEach((data) => {
                if (data.fqn.indexOf("org.hyperledger.composer") == -1){ transactionList.push(data) }
            })

            modelManager.getConceptDeclarations().forEach((data) => {
                if (data.fqn.indexOf("org.hyperledger.composer") == -1){ conceptList.push(data) }
            })


            assetList.forEach((asset) => { assetServiceNames.push(asset.name + 'Service'); });
            ...
            for (let x = 0 ; x < transactionList.length; x++){
                ...
                participantList = orangeUtils.mergeSuperType(participantList);
                assetList = orangeUtils.mergeSuperType(assetList);


                participantList = orangeUtils.removeOptionalValue(participantList);
                assetList = orangeUtils.removeOptionalValue(assetList);
                transactionList = orangeUtils.removeOptionalValue(transactionList)

                participantList = orangeUtils.fillProperties(participantList, conceptList);
                assetList = orangeUtils.fillProperties(assetList, conceptList);
                transactionList = orangeUtils.fillTranascationProperties(transactionList,conceptList, participantList, assetList)
            ...
            assetList = [], assetComponentNames = [] , assetServiceNames = [], transactionServiceNames = [];
            transactionComponentNames = [], transactionList = []
            resolve();
        });
        return createdApp.then(() => { console.log('Created application!'); });
    },

    install: function () {
        if (!skipInstall) {
            return this.installDependencies({ bower: false, npm: true });
        } else { console.log('Skipped installing dependencies'); }
    },
